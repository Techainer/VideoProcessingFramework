  ...
  upDemuxerThread.reset(new thread(
      [](atomic<int> &stopFlag, DemuxerThreadData tData) {
        while (1 != stopFlag.load()) {
          auto buffer = getElementaryVideo(tData.pDemuxer, nullptr, false);
          if (!buffer) {
  ...
  do {
    try {
      if (ctx.IsFlush()) {
        ...
        pRawSurf = getDecodedSurface(upDecoder.get(), upDemuxer.get(), upPacketQueue.get(),
                                     dec_error, eos_flag, ctx.GetSeekContextMutable(), ctx.HasSEI());
      }
