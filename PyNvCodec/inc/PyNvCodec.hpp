...
class PyNvDecoder {
  ...
  std::unique_ptr<ConcurrentQueue<enc_packet>> upPacketQueue;
  std::unique_ptr<thread> upDemuxerThread;
  std::unique_ptr<DemuxFrame> upDemuxer;
...
public:
  ...
  // TODO: Parse SeekContext *seek_ctx, bool needSEI to all getElementaryVideo call
  // It can be extract like this: ctx.GetSeekContextMutable(), ctx.HasSEI()
  // which ctx is DecodeContext& ctx, but since it was parse at call time
  // not when the PyNvDecoder was contructed so we cannot use it for now
